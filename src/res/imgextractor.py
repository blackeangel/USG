...
class Extractor(object):
    ...
    def __ext4extractor(self):
        ...
        fs_config_file = self.BASE_DIR + self.FileName + '_statfile.txt'
        contexts = self.BASE_DIR + self.FileName + "_file_contexts" #08.05.18
        # fs_config_file = 'fs_config.txt'
        def scan_dir(root_inode, root_path="", extract_list=None):
            for entry_name, entry_inode_idx, entry_type in root_inode.open_dir():
                if entry_name in ['.', '..', 'lost+found'] or entry_name.endswith(' (2)'):
                        continue
                # 18/09/18
                if extract_list and not entry_name in extract_list:
                        continue
                # 18/09/18 <---
...
#                        os.chown(dir_target, uid, gid)#14/09/18
                    scan_dir(entry_inode, entry_inode_path)
                    if cap == '' and con == '':
                        self.fsconfig.append('%s %s %s %s' % (self.DIR + entry_inode_path, uid, gid, mode))
                    else:
                        if cap == '':
                            self.fsconfig.append('%s %s %s %s %s' % (self.DIR + entry_inode_path, uid, gid, mode, con))
                            ...
                            if con == '':
                                ...
                                self.fsconfig.append('%s %s %s %s %s' % (self.DIR + entry_inode_path, uid, gid, mode + cap, con))
                                if con != 'u:object_r:' + self.FileName + '_file:s0':#11.05.18
                                    self.context.append('/%s %s' % (self.DIR + entry_inode_path, con))
...
#                        os.chown(file_target, uid, gid) #14/09/18
                    if cap == '' and con == '':
                        self.fsconfig.append('%s %s %s %s' % (self.DIR + entry_inode_path, uid, gid, mode))
                    else:
                        if cap == '':
                    ...
                    try:
                        ...
                        if cap == '' and con == '':
                            ...
                            if cap == '':
                                self.fsconfig.append('%s %s %s %s %s %s' % (self.DIR + entry_inode_path, uid, gid, mode, con, link_target))
                                if con != 'u:object_r:' + self.FileName + '_file:s0':#11.05.18
                                    self.context.append('/%s %s' % (self.DIR + entry_inode_path, con))
                            else:
                                if con == '':
                                    ...
                                    self.fsconfig.append('%s %s %s %s %s %s' % (self.DIR + entry_inode_path, uid, gid, mode + cap, con, link_target))
                                    if con != 'u:object_r:' + self.FileName + '_file:s0':#11.05.18
                                        self.context.append('/%s %s' % (self.DIR + entry_inode_path, con))
                        ...
                        if os.name == 'posix':
                            ...
                            os.chmod(link_target, int(mode, 8))
                            os.chown(link_target, uid, gid)
                        if os.name == 'nt':
        ...
        with open(self.OUTPUT_IMAGE_FILE, 'rb') as file:
            ...
            #26/09/18 -->
            if 'dev' in dirlist or 'd' in dirlist:
                self.EXTRACT_DIR = os.path.realpath(os.path.dirname(self.EXTRACT_DIR))
                scan_dir(root, extract_list=['system'])
            else:
                scan_dir(root)
            #26/09/18 <---
            self.fsconfig.sort()
            for i in range(len(self.fsconfig)): #14/09/18
                self.fsconfig[i]=self.fsconfig[i].replace('system/system','system').replace('vendor/vendor','vendor')#14/09/18
            self.__appendf('\n'.join(self.fsconfig), fs_config_file)
            if self.context:#11.05.18
                self.context.sort()#11.05.18
                for j in range(len(self.context)):#14/09/18
                    self.context[j] = self.context[j].replace('system/system', 'system').replace('vendor/vendor','vendor')#14/09/18
